...
export default function Home({ posts }) {
  return (
    <>
      ...
      <div>
        {!posts.length && <p className="text-muted dark:text-dark-muted">No posts found.</p>}
        {posts.slice(0, MAX_DISPLAY).map((post) => {
          const { slug, date, title, summary, tags } = post
          return (
            <article key={slug} className="border-divider dark:border-dark-divider border-t py-10">
