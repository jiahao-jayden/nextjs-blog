...
修改外围代码只是第一步，真正的难点在于：**如何证明这个修改真的更好，而不是“修了一个 bug，搞砸了整个系统”？**

我对评测（Evals）这块其实理解并不深，但日常看各种评测时（比如腾讯开源的 **WorkBuddy Bench** 这类面向 Coding / 工作流 Agent 的基准），能明显感觉到评测 Agent 和传统软件跑单元测试完全不是一回事。

在真实的自动化演化场景里，至少会撞上几个很现实的问题：

1. **过拟合单次失败轨迹**：针对这次出差订票写出来的拦截补丁，换一个表达方式、换一个城市，或者换成代码开发任务，很可能就失效了；
2. **误拦截与功能退化（Regression）**：为了防止超预算加上了严格拦截，结果导致用户后来正常调整预算、或者完全没有预算限制的普通任务，大面积被错误拦截；
3. **随机性导致“看起来跑通了”**：大模型的输出本身有随机波动，跑通一次用例不代表规则真的具备确定性。

像 WorkBuddy Bench 这类基准给我的启发是：**评测 Agent 不能只看单轮对话回答得怎么样，而是要把 Agent 放在真实的 Docker 沙箱里，跑真实的工作流任务，最后对账物理产物（代码补丁、生成的文件、执行结果）是否正确。**

如果把这个思路映射到我们刚才的订票案例中，外循环在采纳新规则前，至少要跑几组最朴素的对照用例：
- **预算场景**：3000 元固定预算、中途改到 5000 元、经历多次上下文压缩的长会话；
- **非预算场景**：普通的自由订票、写代码、查文档等通用任务，确保没有产生误拦截。

只有在这些基础用例上验证过“既修补了问题，又没有引入误杀”，自动演化修改出来的规则才敢真正合并进运行时的代码库里。
